...
const resolvers = {
  Query: {
    ...
    deleteTVShow: (_, { id }) => {
      return new Promise((resolve, reject) => {
        clientMovies.deletetvShows({ movie_id: id }, (err, response) => {
          if (err) {
            reject(err);
          } else {
    ...
    editTVShow: (_, { id, title, description }) => {
      return new Promise((resolve, reject) => {
        clientMovies.edittvShows(
  ...
  Mutation: {
    ...
    deleteTvshow: (_, { id }) => {
      return new Promise((resolve, reject) => {
        clientTVShows.deleteTvshow({ tv_show_id: id }, (err, response) => {
          if (err) {
            reject(err);
          } else {
            resolve(response.success);
